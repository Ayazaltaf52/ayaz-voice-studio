...
// Optimized: Safe chunk limit (4096 chars) prevents network timeouts while maintaining context
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 pt-16 md:pt-8">
                  <div className="max-w-2xl w-full bg-[#1e1f22] border border-[#2d2e31] rounded-3xl p-10 shadow-2xl animate-in zoom-in-95 duration-300">
                      ...
                      <div className="space-y-6">
                          ...
                          <button onClick={handleCreateClone} disabled={isCloning || !cloneName || uploadedFiles.length === 0} className={`w-full py-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${isCloning ? 'bg-[#2d2e31] text-[#8e9196]' : 'bg-white hover:bg-[#e0e0e0] text-black shadow-lg'}`}>
                              {isCloning ? <><Loader2 className="w-4 h-4 animate-spin" /> Training Voice Model...</> : <><Sparkles className="w-4 h-4 fill-current" /> Train Voice</>}
                          </button>
                      </div>
                  </div>
              </div>
